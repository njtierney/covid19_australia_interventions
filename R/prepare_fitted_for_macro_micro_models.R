...
#' @export
prepare_fitted_for_macro_micro_models <- function(mobility_fitted_nsw_concordance) {
  mobility_fitted_nsw_concordance %>%
    filter(grepl("^Google: ", datastream)) %>%
    mutate(
      change = 1 + (predicted_trend / 100),
      lga_datastream = glue("{LGA_NAME16} {datastream}")
    ) %>%
    ...
    mutate(lga = str_remove(lga_datastream,
                            paste0("\\s", datastream))) %>%
    select(-lga_datastream, -datastream) %>% 
           # -state) %>%
    arrange(lga,
            date) %>%
    ...
    relocate(lga,
             .before = everything()) %>%
    mutate_at(vars(public, home, retail, transit, work),
              ~ replace_na(., 1)) %>%
